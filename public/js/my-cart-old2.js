...
class Checkout extends BaseCart{
    constructor() {
        ...
        this.showErrorModalMessageIfSessionError();
        //this.checkCartItemsAndCoupon();
        this.renderListOfProductsWithTotals();
        //console.log(this)
        this.form;
        this.addListenersHttp();
    ...
    renderListOfProductsWithTotals() {
        //super.renderListOfProductsWithTotals();
    ...
    addListenersHttp(){
        ...
        document.getElementById('cod_button').addEventListener('click', (e)=>{
            e.preventDefault();
            //let confirmation_modal=document.getElementById('confirmation_modal');
            document.getElementById('confirmation_modal_body').textContent='Are you sure that you want to order these products and pay with CASH ON DELIVERY? Click "Confirm" to finish your order.';
                ...
                axios.post(this.route, formData).then((data)=>{
                    ...
                    if(data.data.hasOwnProperty('errors')){
                        ...
                        if(data.data.hasOwnProperty('products_in_cart')){
                            ...
                            this.cart_items=data.data.products_in_cart;
                            this.calculateTotalsAndCartQty();//ok
                            this.setStorageContent(); //ok
                            this.renderListOfProductsWithTotals();//ok
                            this.showCartQuantityInNav();//ok
                        }
                        if(data.data.errors.hasOwnProperty('items')){
                            let resp_object=data.data.errors.items;
                            let ids=Object.keys(resp_object);
        ...
        document.getElementById('card_button').addEventListener('click', (e)=>{
            e.preventDefault();
            document.getElementById('confirmation_modal_body').textContent='Are you sure that you want to order these products and pay with CREDIT CARD? Click "Confirm" to finish your order.';
            ...
            document.getElementById('confirm_modal_button').addEventListener('click', (e)=>{
                ...
                //let formData=new FormData();
                this.form=document.getElementById('checkout_form');
                this.createAdditionalFieldInForm('items', JSON.stringify(this.cart_items));
               this.createAdditionalFieldInForm('payment_type', 'card');
               if(this.coupon_is_applied){
                   this.createAdditionalFieldInForm('coupon', JSON.stringify(this.coupon));
               }
                $('#confirmation_modal').modal('hide');
               this.form.submit();
    ...
    addListeners1(){
        ...
        document.getElementById('card_button').addEventListener('click', (e)=>{
            ...
            document.getElementById('confirm_modal_button').addEventListener('click', (e)=>{
                ...
                let formData=new FormData();
                let names=['confirm_terms', 'name', 'contact_person','email', 'address', 'city', 'zip', 'phone1', 'phone2','comment'];
                let card_names=['card', 'e_month', 'e_year', 'cvc'];
...
console.log(data)
    ...
    //    this.createAdditionalFieldInForm('items', JSON.stringify(this.cart_items));
    //    this.createAdditionalFieldInForm('payment_type', 'cod');
    //    if(this.coupon_is_applied){
    //        this.createAdditionalFieldInForm('coupon', JSON.stringify(this.coupon));
    //    }
    //    //this.form.submit();moraće ajax
    // }
    //
    // createAdditionalFieldInForm(name, value){
    //     let input=document.createElement('input');
